...
@Component({
  selector: 'app-python-modul',
  templateUrl: './python-modul.component.html',
  styleUrls: ['./python-modul.component.css']
})
export class PythonModulComponent implements AfterViewInit{
  product: any;
      ...
      this.langTranslate.jsonData$.pipe(
        map(data => {
          ...
          for (let i = 0; i < data.length; i++) {
            ...
            if (planItem.pythonnModules) {
              product.push(...planItem.pythonnModules);
            }
          }
          return product;
        })
